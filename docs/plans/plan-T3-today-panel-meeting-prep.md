# T3 — Today panel and meeting-prep pipeline
...
ticket adds the Today panel: the same prep work, linked to the right account,
...
- The Today panel shows a card for each of today's external customer meetings,
  with the same prep content the standalone document has today (agenda, risks,
  history), now linked to the correct Book of Business account.
...
- **Artifact:** the `meetings` entity and its panel UI, new in this ticket.
...
**`src/artifact/command-center.html`** — add the Today panel: one card per
`meetings[today][*]` entry, showing the brief and an editable notes field
bound to `meetings[today][accountId].notes`. Add the panel's refresh button
(same ask-Claude mechanism as T2's, same open question about capability
availability).
...
| `src/artifact/command-center.html` | Add Today panel UI and refresh button. |

## Verification

1. Run the updated skill on a real day with at least one external meeting.
   Confirm the brief appears in the Today panel, linked to the correct
   account.
